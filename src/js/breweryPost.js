  ...
  for (let i = 0; i < breweryListPerState.length; i++) {
    ...
    if(i === breweryListPerState.length-1) {
      for (let j = 0; j < breweryResult.length ; j++) { 
        console.log('j' + j);
        $(selector).append('<li class=' + 'unique' + '><strong>' + breweryResult[j].name.name + '</strong></li>');
        $(selector).append('<ul class=' + 'unique' + '>Distance: ' + breweryResult[j].distance.toFixed(1) + ' Miles</ul>');
        $(selector).append('<ul class=' + 'unique' + '>Address: ' + breweryResult[j].name.street + ', ' + breweryResult[j].name.city + ', ' + breweryResult[j].name.zip + '</ul>');
        $(selector).append('<ul class=' + 'unique' + '>Website: <a href="' + breweryResult[j].name.url + '">' + breweryResult[j].name.url + '</a></ul>');
      }
    }
  }
}