...
export default class BreweryFunctions {
  ...
  static breweryPost(sortedBreweriesByDistance, selector) {
    ...
    for (let i = 0; i < sortedBreweriesByDistance.length ; i++) {
      $(selector).append('<li class=' + 'unique' + '><strong>' + sortedBreweriesByDistance[i].name.name + '</strong></li>');
      $(selector).append('<ul class=' + 'unique' + '>Distance: ' + sortedBreweriesByDistance[i].distance.toFixed(1) + ' Miles</ul>');
      $(selector).append('<ul class=' + 'unique' + '>Address: ' + sortedBreweriesByDistance[i].name.street + ', ' + sortedBreweriesByDistance[i].name.city + ', ' + sortedBreweriesByDistance[i].name.zip + '</ul>');
      $(selector).append('<ul class=' + 'unique' + '>Website: <a href=https://www.' + sortedBreweriesByDistance[i].name.url.toString() + '>' + sortedBreweriesByDistance[i].name.url + '</a></ul>');
    }
  }
}